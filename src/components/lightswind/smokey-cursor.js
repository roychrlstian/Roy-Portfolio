...
export default function SmokeyCursor({ simulationResolution = 128, dyeResolution = 1440, captureResolution = 512, densityDissipation = 3.5, velocityDissipation = 2, pressure = 0.1, pressureIterations = 20, curl = 3, splatRadius = 0.2, splatForce = 6000, enableShading = true, colorUpdateSpeed = 10, backgroundColor = { r: 0.5, g: 0, b: 0 }, transparent = true, className = "", disabled = false, intensity = 1, followMouse = true, autoColors = true, }) {
    ...
    `);
        ...
        // Animation frame id and running flag for cleanup
        let rafId = 0;
        let running = true;
        ...
        function generateColor() {
            // Produce only blue-ish hues.
            // Hue ~0.6-0.7 corresponds to blue; vary saturation/value slightly for variety.
            const hue = 0.62 + (Math.random() - 0.5) * 0.12; // ~0.56 - 0.68
            const sat = 0.6 + Math.random() * 0.4; // 0.6 - 1.0
            const val = 0.6 + Math.random() * 0.4; // 0.6 - 1.0
            const c = HSVtoRGB(hue, sat, val);
            // Keep the original subtle intensity scaling
            c.r *= 0.15;
            c.g *= 0.15;
            c.b *= 0.15;
            return c;
        ...
        // ------------------------------------------------------------
        // Cleanup function: remove listeners, cancel animation
        function cleanup() {
            running = false;
            if (rafId)
            ...
            // Remove the other window listeners by using named functions where possible
            // (We can't remove anonymous handlers above easily here.)
        }
        // Attach cleanup to effect return via closure
        return cleanup;
    }, [
